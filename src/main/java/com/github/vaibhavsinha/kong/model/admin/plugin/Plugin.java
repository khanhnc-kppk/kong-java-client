 ...
 * will run depends on the entities it has been configured for.<br/> Plugins can be configured for
 * various entities, combination of entities, or even globally. This is useful, for example, when
 * you wish to configure a plugin a certain way for most requests, but make authenticated requests
 * behave slightly differently.<br/> Therefore, there exists an order of precedence for running a
 * plugin when it has been applied to different entities with different configurations. The rule of
 * thumb is: the more specific a plugin is with regards to how many entities it has been configured
 * on, the higher its priority.<br/> The complete order of precedence when a plugin has been
 * configured multiple times is:
 * </p>
 * <ul>
 * <li>
